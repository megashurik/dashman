...
# sibman - main executable
# installs, updates, and manages sib daemons and wallets
...
if [[ ${BASH_VERSION%%.*} != '4' ]];then
    die "sibman requires bash version 4. please update. exiting."
fi
...
SIBMAN_BIN=$(readlink -f $0)
SIBMAN_GITDIR=$(readlink -f ${SIBMAN_BIN%%/bin/${SIBMAN_BIN##*/}})
source $SIBMAN_GITDIR/lib/sibman_functions.sh
...
source $SIBMAN_GITDIR/lang/en_US.sh

# override if configured
lang_type=${LANG%%\.*}
[[ -e $SIBMAN_GITDIR/lang/$lang_type.sh ]] && source $SIBMAN_GITDIR/lang/$lang_type.sh

# process switch overrides ---------------------------------------------------

# show version and exit if requested
[[ $VERSION || $1 == 'version' ]] && echo $SIBMAN_VERSION && exit 0
...
echo -e "${C_CYAN}${messages["sibman_version"]} $SIBMAN_VERSION$SIBMAN_CHECKOUT${C_NORM} - ${C_GREEN}$(date)${C_NORM}"
...
case "$1" in
        restart)
            ...
            _find_sib_directory
            _check_sibd_state
            # TODO, show uptime: ps --no-header -o pid,etime $(cat $INSTALL_DIR/sib.pid) | awk '{print $2}'
            case "$2" in
                now)
                    restart_sibcoind
                    ;;
        ...
        sync)
            ...
            if [ -z $(git config user.email) ] ; then
                git config user.email "sibmanuser"
                git config user.name "sibmanuser"
            fi
            git stash
            git checkout master
            git reset --hard origin/master

            if [ -e $SIBMAN_GITDIR/PREVIOUS_VERSION ]; then
                echo '--------------'
                cat_until "^$( cat $SIBMAN_GITDIR/PREVIOUS_VERSION ) " $SIBMAN_GITDIR/CHANGELOG.md | sed \
                ...
                echo '--------------'
                rm $SIBMAN_GITDIR/PREVIOUS_VERSION
            fi

            if [ ! -z "$2" ]; then
                self=${0##*/};
                shift;
                exec $SIBMAN_GITDIR/$self $@
            fi
        ...
        vote)
            ...
            ok " ${messages["done"]}"
            echo
            export SIBCOIN_CLI SIBMAN_PID=$$
            /usr/bin/env python $SIBMAN_GITDIR/bin/sibvote.py
            quit 'Exiting.'
            ;;
        status)
            COMMAND=$1
