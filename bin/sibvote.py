...
git_dir = os.path.abspath(
    os.path.join(
        ...
        '..'))
sib_conf_dir = os.path.join(os.getenv('HOME'), '.sibcore')
sib_cli_path = os.getenv('SIBCOIN_CLI')
if os.getenv('SIBMAN_PID') is None:
    quit("--> please run using 'sibman vote'")
...
def run_sib_cli_command(cmd):
    return run_command("%s %s" % (sib_cli_path or 'sibcoin-cli', cmd))
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        ...
        for mn in shuffled_mn_keys:
            ...
            if background_send:
                deferred_votes = tempfile.NamedTemporaryFile(prefix=('sending_sibvote_votes-%s=' % batch_timestamp),delete=False)
            ...
            for vote in shuffled_vote_keys:
                ...
                signature = sibutil.sign_vote(netvote, mnprivkey)
                command = ('%s' % sib_cli_path is not None and sib_cli_path or 'sibcoin-cli') + ' voteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(
        ...
        if background_send:
            voter_parent = tempfile.NamedTemporaryFile(prefix=('sending_sib_votes-%s-' % batch_timestamp), delete=False)
...
def main(screen):
    ...
    if sib_cli_path is None:
        # test sibcoin-cli in path -- TODO make robust
        try:
            run_command('sibcoin-cli getinfo')
        except subprocess.CalledProcessError:
            quit(
                "--> cannot find sibcoin-cli in $PATH\n" +
                "    do: export PATH=/path/to/sibcoin-cli-folder:$PATH\n" +
                "    and try again\n")

    loadwin = curses.newwin(40, 40, 1, 2)

    loadwin.addstr(1, 2, 'sibvote version: ' + version, C_CYAN)

    mncount = int(run_sib_cli_command('masternode count enabled'))
    block_height = int(run_sib_cli_command('getblockcount'))
    ...
    # get ballot
    ballots = json.loads(run_sib_cli_command('gobject list all'))
    ballot = {}
    ...
    for entry in ballots:
        ...
        ballots[entry][u'votes'] = json.loads(run_sib_cli_command('gobject getvotes %s' % entry))
    ...
    masternodes = {}
    with open(os.path.join(sib_conf_dir, 'masternode.conf'), 'r') as f:
        lines = list(
            line
            for line in
            (l.strip() for l in f)
        ...
        for line in lines:
            ...
            masternodes[conf[3] + '-' + conf[4]] = {
                ...
                "txout": conf[4]}
    if not masternodes:
        # fallback to sibcoin.dash.conf entries if no masternode.conf entries
        with open(os.path.join(sib_conf_dir, 'sibcoin.conf'), 'r') as f:
            lines = list(
                line
                for line in
                (l.strip() for l in f)
            ...
            if all(k in conf for k in ('masternode', 'externalip', 'masternodeprivkey')):
                ...
                quit('cannot find masternode information in sibcoin.conf')
    ...
    votewin.addstr(1, 2, 'sibvote version: ' + version, C_CYAN)
    votewin.addstr(
        2,
        2,
